...
const edit = ({router}) => {
  ...
  const renderTextFields = () => {
   ...
   console.log({courseFormMap})

    return Object.keys(courseFormMap).map(field => {
      return (
...
const handleSelectFile = async (e) =>{
  ...
  let downloadURL = await uploadTaskSnapshot.ref.getDownloadURL()
   ...
   await firestore.collection('courses').doc(id).collection('course_videos').doc(imageName).set({
...
useEffect(()=>{
 ...
 const getVideosForCourse = async () =>{
   if(!_.isEmpty(id)&&(!loadingVideo)){
     const courseVideosRef = firestore.collection('courses').doc(id).collection('course_videos')
  ...
  return (
   ...
   <input  onChange={handleSelectFile}  id='videoUpload' type='file'></input>
    ...
    <button  onClick={handleSubmit} disabled={!fieldsChanged} >Save Record</button>
